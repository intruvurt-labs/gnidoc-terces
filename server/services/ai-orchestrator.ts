...
export class AIOrchestrator {
  ...
  private generateDemoCode(prompt: string, options?: any): string {
    ...
    if (prompt.toLowerCase().includes('todo') || prompt.toLowerCase().includes('task')) {
      return `// React TypeScript Todo Application (Generated by GINDOC)
...
// This is a demo response since Google AI API key is not configured
...
const GeneratedComponent: React.FC = () => {
  return (
    <div style={{ padding: '20px', textAlign: 'center' }}>
      ...
      <p>Prompt: ${prompt}</p>
      <p>This is demonstration code generated by GINDOC.</p>
      <p>To enable full AI generation, configure your Google AI API key.</p>
    </div>
  );
};
